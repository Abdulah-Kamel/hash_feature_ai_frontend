    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <Button
          onClick={handleBack}
          variant="outline"
          className="rounded-full py-5 px-4 bg-card cursor-pointer"
        >
      ...
      <Card
        className={`rounded-2xl bg-gradient-to-b ${cardGradient} text-white p-8 ${
          bgClassName || ""
        }`}
      >
            ...
            <div
              ...
              style={{
                background: `conic-gradient(#fff ${
                  percent * 3.6
                }deg, rgba(255,255,255,.25) 0deg)`,
              }}
            />
            <div
              className={`absolute inset-2 rounded-full ${ringBg} grid place-items-center`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card
          className="rounded-xl bg-card p-4 flex flex-row items-center gap-1"
          dir="rtl"
        >
      ...
      <div className="space-y-3">
        <Button
          onClick={onRestart}
          className={`w-full h-14 rounded-full ${restartBtnBg} hover:${restartBtnBg} cursor-pointer`}
        >
          إعادة الأختبار
        </Button>
        {!isTest && (
          <Button
            onClick={onNextStage}
            className="w-full h-14 rounded-full cursor-pointer bg-primary hover:bg-primary/90"
          >
            المرحلة التالية
          </Button>
        )}
